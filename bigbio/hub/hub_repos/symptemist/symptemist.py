...
_HOMEPAGE = "https://zenodo.org/record/8223654"

_LICENSE = 'CC_BY_4p0'

_URLS = {
    _DATASETNAME: "https://zenodo.org/record/8223654/files/symptemist_train.zip?download=1",
}
...
class SymptemistDataset(datasets.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager) -> List[datasets.SplitGenerator]:
        ...
        base_bath = Path(data_dir) / "symptemist_train"
    ...
    def _generate_examples(
        ...
        print("track:", track, "split:", split)

        tsv_files = {
            ('entities', 'train'): [
                base_bath / "subtask1-ner" / "tsv" / "symptemist_tsv_train_subtask1.tsv"
            ],
            ('entities', 'test'): [
                base_bath / "test_annotated" / "subtrack1_entities" / "symptemist_subtrack1_test_mentions.tsv"
            ],
            ('linking', 'train'): [
                base_bath / "training" / "subtrack2_linking" / "symptemist_subtrack2_training1_linking.tsv",
                base_bath / "training" / "subtrack2_linking" / "symptemist_subtrack2_training2_linking.tsv",
            ],
            ('linking', 'test'): [
        ...
        if split == "train":
            text_files_dir = base_bath / "subtask1-ner" / "txt"
        #elif split == "test":
        ...
        for uid, filename in enumerate(entity_file_names):
            ...
            for row in entities_df.itertuples(name="Entity"):
                entity = {
                    "id": f"{uid}_{row.filename}_{row.Index}_entity_id_{row.ann_id}",
                    "type": row.label,
                    "text": [row.text],
